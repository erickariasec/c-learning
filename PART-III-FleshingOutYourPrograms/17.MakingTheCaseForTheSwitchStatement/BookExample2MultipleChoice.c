...
When the user makes her second choice, it prints a list of key information from that specific decade in that
specific category. */
...
/*
Nesting switch statements is not a good idea, particularly when the default choices start overlapping. It confuses
your compiler, and the program will not run.

Another note is that, in the first program, you did not enter open and closing braces for the statements after each
case (expression): statement, but here you did. The braces are not needed, but with more complex blocks of code, the
braces can help keep things clear. */

/*
TIP:
You can replace the repeated code sections in the second-level menus with single lines of code when you learn to
write your own functions later in the book. */
